...
class WhatsAppWatcher(BaseWatcher):
    def __init__(self, vault_path: str, session_path: str = None, keywords: list = None, test_mode: bool = False):
        super().__init__(vault_path, check_interval=30, test_mode=test_mode)  # Check every 30 seconds
    ...
    def check_for_updates(self) -> list:
        ...
        try:
            with sync_playwright() as p:
                ...
                if self.test_mode:
                    print("BROWSER LAUNCH: Starting WhatsApp Web browser for authentication setup...")
                ...
                if self.test_mode:
                    print("BROWSER LAUNCH: WhatsApp Web browser started with persistent context")

                page = browser.new_page()

                if self.test_mode:
                    print("QR LOGIC: Navigating to WhatsApp Web...")

                page.goto('https://web.whatsapp.com')

                # Wait for WhatsApp to load and check if logged in
                try:
                    # Wait for chat list or QR code to appear
                    page.wait_for_selector('[data-testid="chat-list"], [data-testid="qr-code"]', timeout=15000)

                    # Check if QR code is present (not logged in) or chat list is present (logged in)
                    qr_code = page.query_selector('[data-testid="qr-code"]')
                    chat_list = page.query_selector('[data-testid="chat-list"]')

                    if qr_code:
                        if self.test_mode:
                            print("QR LOGIC: WhatsApp Web requires QR scan. Please scan QR code in the browser.")
                            print("QR LOGIC: Wait for successful login and chat list to appear.")
                            input("Press Enter after you have logged in successfully...")
                        else:
                            self.logger.warning("WhatsApp Web not logged in (QR code shown), skipping check")
                            browser.close()
                            return []
                    elif chat_list:
                        if self.test_mode:
                            print("QR LOGIC: WhatsApp Web authentication confirmed!")
                    else:
                        # Unknown state, try to continue
                        if self.test_mode:
                            print("QR LOGIC: Could not determine login status, proceeding...")

                except Exception as login_check_error:
                    if self.test_mode:
                        print(f"QR LOGIC: Error during login check: {login_check_error}")
                    self.logger.warning("Could not confirm WhatsApp Web login status, proceeding anyway")
                ...
                if self.test_mode:
                    print(f"SESSION PERSISTENCE: Found {len(chat_items)} chats to check...")

                for chat_item in chat_items:
                    # Check if chat has unread messages - look for unread indicators
                    unread_indicator = chat_item.query_selector('span[data-testid="default-message-status"]')
                    if unread_indicator:
                        # Extract chat info
                        ...
                        if chat_name_elem:
                            ...
                            # Get recent messages (last 10 messages to catch more context)
                            message_bubbles = page.query_selector_all('[data-testid="msg"]')

                            if self.test_mode:
                                print(f"SESSION PERSISTENCE: Found {len(message_bubbles)} message bubbles in {chat_name}")

                            for msg_bubble in message_bubbles[-10:]:  # Check last 10 messages to avoid missing content
                ...
                if self.test_mode:
                    print(f"SESSION PERSISTENCE: Found {len(messages_to_process)} messages with keywords to process")
            ...
            if self.test_mode:
                print(f"ERROR: WhatsApp check failed with error: {e}")
            return []
...
def main():
    ...
    watcher = WhatsAppWatcher(vault_path, session_path, test_mode=test_mode)
    watcher.run()
